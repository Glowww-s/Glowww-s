...
def vector_merge(wordfreq_dicts):
    ...
    doc_vectors = [{k: wordfreq_dict.get(k, 0) for k in key_merge} for wordfreq_dict in wordfreq_dicts]  # 合并的文档向量计数
    assert (doc_vectors[0].keys() == doc_vectors[1].keys()), "合并文档向量key不一致"  # 检测合并向量一致性
    return doc_vectors
